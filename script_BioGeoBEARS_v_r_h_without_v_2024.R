...
trfn_v_r_h = np("/Users/marcosqueiroz1/Library/CloudStorage/GoogleDrive-marvin.danque@gmail.com/My Drive/PD/Biogeo/0_trees/2024/parental_v_r_h_ML_calibrated_2024_wo_outgroup.nwk_minBL_rotated.newick")

# Look at the raw Newick file:
moref(trfn_v_r_h)

tr_v_r_h = read.tree(trfn_v_r_h)
...
# (from Ree & Smith 2008)
geogfn = np("geo_without_v_2024.txt")
...
BioGeoBEARS_run_object$trfn = trfn_v_r_h
...
runslow = TRUE
resfn = "./DEC_Curcuma_v_r_h_without_v_2024.Rdata"
if (runslow)
{
...
resfn = "./DEC_J_Curcuma_v_r_h_without_v_2024.Rdata"
runslow = TRUE
if (runslow)
{
...
# PDF plots
#######################################################
pdffn = "./Curcuma_v_r_h_without_v_DEC_vs_DEC+J_2024.pdf"
pdf(pdffn, height=15, width=10)

#######################################################
# Plot ancestral states - DEC
#######################################################
analysis_titletxt ="BioGeoBEARS DEC on Curcuma_v_r_h without_v_2024"
...
# States
res2 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45, tipcex=0.7, statecex=0.7, splitcex=0.6, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr_v_r_h, tipranges=tipranges)

#svgn = "Orchids_New_test_DEC_M0_unconstrained_v1.svg"
#svglite(svgn, height = 15, width = 15)

# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45, tipcex=0.7, statecex=0.7, splitcex=0.6, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr_v_r_h, tipranges=tipranges)
...
#######################################################
analysis_titletxt ="BioGeoBEARS DEC+J on Curcuma_v_r_h without_v 2024"
...
# States
res1 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45, tipcex=0.7, statecex=0.7, splitcex=0.6, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr_v_r_h, tipranges=tipranges)

#svgn = "Orchids_New_test_DEC+J_M0_unconstrained_v1.svg"
#svglite(svgn, height = 15, width = 15)
...
runslow = TRUE
resfn = "./DIVALIKE_Curcuma_v_r_h_without_v_2024.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "./DIVALIKE_J_Curcuma_v_r_h_without_v_2024.Rdata"
runslow = TRUE
if (runslow)
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
pdffn = "./Curcuma_v_r_h_without_v_DIVALIKE_vs_DIVALIKE+J_2024.pdf"
pdf(pdffn, height=15, width=10)

#######################################################
# Plot ancestral states - DIVALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE on Curcuma_v_r_h 2024"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on Curcuma_v_r_h 2024"
...
runslow = TRUE
resfn = "./BAYAREALIKE_Curcuma_v_r_h_without_v_2024.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "./BAYAREALIKE_J_Curcuma_v_r_h_without_v_2024.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = "./Curcuma_v_r_h_without_v_BAYAREALIKE_vs_BAYAREALIKE+J_2024.pdf"
pdf(pdffn, height=15, width=10)

#######################################################
# Plot ancestral states - BAYAREALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on Curcuma_v_r_h 2024"
...
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on Curcuma_v_r_h 2024"
...
# Loads to "restable"
save(restable, file="./Curcuma_v_r_h_without_v_restable.Rdata")
load(file="./Curcuma_v_r_h_without_v_restable.Rdata")

# Loads to "teststable"
save(teststable, file="./Curcuma_v_r_h_without_v_teststable.Rdata")
load(file="./Curcuma_v_r_h_without_v_teststable.Rdata")

# Also save to text files
write.table(restable, file="./Curcuma_v_r_h_without_v_restable.txt", quote=FALSE, sep="\t")
write.table(unlist_df(teststable), file="./Curcuma_v_r_h_without_v_teststable.txt", quote=FALSE, sep="\t")
...
samplesize = length(tr_v_r_h$tip.label)
...
# Also save to text files
write.table(restable_AIC_rellike, file="./Curcuma_v_r_h_without_v_restable_AIC_rellike.txt", quote=FALSE, sep="\t")
write.table(restable_AICc_rellike, file="./Curcuma_v_r_h_without_v_AICc_rellike.txt", quote=FALSE, sep="\t")

# Save with nice conditional formatting
write.table(conditional_format_table(restable_AIC_rellike), file="./Curcuma_v_r_h_without_v_restable_AIC_rellike_formatted.txt", quote=FALSE, sep="\t")
write.table(conditional_format_table(restable_AICc_rellike), file="./Curcuma_v_r_h_without_v_restable_AICc_rellike_formatted.txt", quote=FALSE, sep="\t")
...
# Stochastic mapping
############################################################################
decj_path <- '/Users/marcosqueiroz1/Library/CloudStorage/GoogleDrive-marvin.danque@gmail.com/My Drive/PD/Biogeo/Biogeobears/v_r_h_without_v/Curcuma_v_r_h_without_v_rotated_DEC_J_M0_unconstrained_v1.Rdata'
resDECj <- load(decj_path)

model_name = "DECj"
tr <- tr_v_r_h

#Update my res file
res$inputs$trfn <- "/Users/marcosqueiroz1/Library/CloudStorage/GoogleDrive-marvin.danque@gmail.com/My Drive/PD/Biogeo/0_trees/parental_v_r_h/parental_v_r_h_without_v_ML_calibrated_filtered_rotated.nwk"
res$inputs$geogfn <- "/Users/marcosqueiroz1/Library/CloudStorage/GoogleDrive-marvin.danque@gmail.com/My Drive/PD/Biogeo/Biogeobears/geo_without_v.txt"

#pdffn = paste0("Psychotria_", model_name, "_v1.pdf")
#pdf(pdffn, height=6, width=6)

analysis_titletxt = paste0(model_name, " on Curcuma v_r_h_without_v")
...
trtable = prt(tr_v_r_h, printflag=FALSE)
head(trtable)
tail(trtable)
...
axisPhylo()
nodelabels()
tiplabels(1:length(tr_v_r_h$tip.label))
...
write.table(range_probabilities, file="range_probabilities_Curcuma_v_r_h_without_v_DEC+J_2024.txt", quote=FALSE, sep="\t")

# Look at the file
moref("range_probabilities_Curcuma_v_r_h_without_v_DEC+J_2024.txt")


######
#Stack barcharts for inwards and outwards dispersion of Curcuma
#####
library(reshape2)
library(ggplot2)
library(cowplot)
...
founder_counts = read.table('founder_counts_fromto_means.txt', header = T)



n_of_letters = nrow(founder_counts)

#inwards
inw_founder_counts_melted = melt(founder_counts)
letters = rep(LETTERS[1:n_of_letters], times = n_of_letters)
inw_founder_counts_melted$from = letters
colnames(inw_founder_counts_melted) = c('to', 'values', 'from')
inw_founder_counts_melted = inw_founder_counts_melted[, c('from', 'to', 'values')]
inw_founder_counts_melted$from = as.character(inw_founder_counts_melted$from)


# Plot the stacked bar chart
inw_plot = ggplot(inw_founder_counts_melted, aes(fill = from, y = values, x = to)) +
  geom_bar(stat = "identity", position = "stack", color = 'black') +
  theme_classic() +
  scale_fill_manual(values = c('#D73027', '#DF8D59', '#FEE090', '#FFFFBF', '#E0F3F8', '#91BFDB', '#4575B4')) + #colors from inkscape fig1 
  theme(legend.position = 'none',
        axis.title.y = element_blank()) +
  xlab('Cladogenetic dispersion (sink areas)') +
  ylim(0, 4)

#outwards
out_founder_counts_melted = as.data.frame(t(founder_counts))
out_founder_counts_melted = melt(out_founder_counts_melted)
out_founder_counts_melted$to = letters
colnames(out_founder_counts_melted) = c('from', 'values', 'to')
out_founder_counts_melted = out_founder_counts_melted[, c('from', 'to', 'values')]
out_founder_counts_melted$from = as.character(out_founder_counts_melted$from)

# Plot the stacked bar chart
out_plot = ggplot(out_founder_counts_melted, aes(fill = to, y = values, x = from)) +
  geom_bar(stat = "identity", position = "stack", color = 'black') +
  theme_classic() +
  scale_fill_manual(values = c('#D73027', '#DF8D59', '#FEE090', '#FFFFBF', '#E0F3F8', '#91BFDB', '#4575B4')) + #colors from inkscape fig1
  theme(legend.position = 'none') +
  xlab('Cladogenetic dispersion (origin areas)') +
  ylab('frequency')



#plot_grid(out_plot, inw_plot, align = 'h', labels = c('(a)', '(b)'))


# svglite::svglite('./dispersion_plots.svg', width = 8.3, height = 3.9)
# plot_grid(inw_plot, out_plot, align = 'h')
# dev.off()

######
#Stack barcharts for sympatric dispersion of Curcuma (d parameters)
#####

ana_dispersal = read.table('ana_dispersals_counts_fromto_means.txt', header = T)

#stack bars
n_of_letters = nrow(ana_dispersal)

#anagenetc origin
ana_origin_melted = melt(ana_dispersal)
letters = rep(LETTERS[1:n_of_letters], times = n_of_letters)
ana_origin_melted$from = letters
colnames(ana_origin_melted) = c('to', 'values', 'from')
ana_origin_melted = ana_origin_melted[, c('from', 'to', 'values')]
ana_origin_melted$from = as.character(ana_origin_melted$from)


# Plot the stacked bar chart
ana_origin_plot = ggplot(ana_origin_melted, aes(fill = from, y = values, x = to)) +
  geom_bar(stat = "identity", position = "stack", color = 'black') +
  theme_classic() +
  scale_fill_manual(values = c('#D73027', '#DF8D59', '#FEE090', '#FFFFBF', '#E0F3F8', '#91BFDB', '#4575B4')) + #colors from inkscape fig1 
  theme(legend.position = 'none') +
  xlab('Anagenetic dispersion (origin areas)') +
  ylab('frequency') +
  ylim(0, 3)
...
#anagenetic source
ana_source_melted = as.data.frame(t(ana_dispersal))
ana_source_melted = melt(ana_source_melted)
ana_source_melted$to = letters
colnames(ana_source_melted) = c('from', 'values', 'to')
ana_source_melted = ana_source_melted[, c('from', 'to', 'values')]
ana_source_melted$from = as.character(ana_source_melted$from)

# Plot the stacked bar chart
ana_source_plot = ggplot(ana_source_melted, aes(fill = to, y = values, x = from)) +
  geom_bar(stat = "identity", position = "stack", color = 'black') +
  theme_classic() +
  scale_fill_manual(values = c('#D73027', '#DF8D59', '#FEE090', '#FFFFBF', '#E0F3F8', '#91BFDB', '#4575B4')) + #colors from inkscape fig1
  theme(legend.position = 'none',
        axis.title.y = element_blank()) +
  xlab('Anagenetic dispersion (sink areas)') +
  ylim(0, 3)


svglite::svglite('./dispersion_plots.svg', width = 8.3, height = 6)
plot_grid(out_plot, inw_plot, ana_origin_plot, ana_source_plot, labels = c('(a)', '(b)', '(c)', '(d)'))
dev.off()


### List of colors for piecharts for paper

areanames = names(tipranges@df)
statenames = areas_list_to_states_list_new(areas=areanames, maxareas=max_range_size, include_null_range=TRUE, split_ABC=FALSE)

color_list = c('#c0c0c0', '#d73027', '#fc8d59', '#fee090', '#ffffbf', '#e0f3f8',
               '#91bfdb', '#4575b4', rep('#c0c0c0', 18), '#cf9fff', '#c0c0c0',
               '#00ffff', rep('#c0c0c0', 99))

#vrh
svglite('vrh_for_supplement.svg', height = 20, width = 10)
# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), 
                         plotwhat="pie", label.offset=0.45, tipcex=0.7, 
                         statecex=0.4, splitcex=0.4, titlecex=0.8, 
                         plotsplits=TRUE, cornercoords_loc=scriptdir, 
                         include_null_range=TRUE, tr=tr_v_r_h,
                         tipranges=tipranges,
...
#cvr
# Pie chart
svglite('cvr_for_supplement.svg', height = 20, width = 10)
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), 
                         plotwhat="pie", label.offset=0.45, tipcex=0.7, 
                         statecex=0.4, splitcex=0.4, titlecex=0.8, 
                         plotsplits=TRUE, cornercoords_loc=scriptdir, 
                         include_null_range=TRUE, tr=tr_c_v_h,
                         tipranges=tipranges,
                         colors_list_for_states = color_list)
dev.off()
